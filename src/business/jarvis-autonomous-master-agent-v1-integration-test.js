...
(async () => {
        ...
        prospectDiscovery: {
            async discover() {
                return {
                    ...
                    status: "COMPLETE",
                    prospects: [
                        { id: "P1", name: "Dubai Test Agency", phone: "+971500000000", website: "https://example.com" }
                    ]
                };
            }
        }
    ...
    const report = await agent.executeGoal({
        ...
        task_input: {
            LEAD_INTAKE: { query: "Dubai real estate", limit: 1 },
            LEAD_QUALIFICATION: { required_contact: "phone_or_website" }
        }
    });
    ...
    const firstTask = taskManager.getTask(report.plan.task_ids[0]);
    const approvalTask = taskManager.getTask(report.plan.task_ids[3]);
    const bookingTask = taskManager.getTask(report.plan.task_ids[4]);
    assert.strictEqual(firstTask.status, "COMPLETED");
    assert.strictEqual(approvalTask.status, "COMPLETED");
