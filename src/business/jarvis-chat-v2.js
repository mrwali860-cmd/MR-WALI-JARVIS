...
class JarvisChatV2 {
  ...
  async respond(message, context = {}) {
    if (!this.client) return { chatMode: "FALLBACK", reply: this.fallbackMessage };
    try {
      const response = await this.client.responses.create({ model: this.model, instructions: SYSTEM_INSTRUCTIONS, input: JSON.stringify({ message, context }) });
