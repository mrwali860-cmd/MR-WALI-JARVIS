...
(async () => {
    const testRoot = path.join(process.cwd(), "data", "test-isolation", "revenue-engine-v1-integration");
    const serviceStorePath = path.join(testRoot, "services.json");
    const taskStorePath = path.join(testRoot, "tasks.json");

    fs.rmSync(testRoot, { recursive: true, force: true });

    try {
        const app = new RevenueEngineV1Integration({
            serviceManager: new ServiceManager({ storePath: serviceStorePath }),
            taskManager: new TaskManager({ storagePath: taskStorePath })
        });
