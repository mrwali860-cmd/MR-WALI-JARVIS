...
(async () => {
    ...
    const offer = app.createOffer({
        opportunity_id: opportunityId,
        offer_id: "offer-integration-001",
        amount: 1500,
        currency: "USD",
        problem: "Slow lead response",
        outcome: "Qualified leads and approved appointment workflow"
    });
    assert.strictEqual(offer.status, "OFFER_SENT");
    assert.strictEqual(app.approveOffer(offer.offer_id).status, "APPROVED");

    assert.throws(
        () => app.confirmPayment({ offer_id: offer.offer_id, payment_status: "PENDING", transaction_reference: "tx-pending" }),
        /payment must be CONFIRMED/
    );

    const payment = app.confirmPayment({
        offer_id: offer.offer_id,
        payment_status: "CONFIRMED",
        transaction_reference: "tx-integration-001",
    ...
    assert.strictEqual(payment.service_plan.service_id, serviceId);
    assert.strictEqual(payment.service_plan.task_count, 10);

    const duplicate = app.confirmPayment({
        offer_id: offer.offer_id,
        payment_status: "CONFIRMED",
        transaction_reference: "tx-integration-001"
    });
    ...
    const execution = await app.executeTask({
        service_id: serviceId,
    ...
    const revenue = app.recordRevenue({
        service_id: serviceId,
        request_id: requestId,
