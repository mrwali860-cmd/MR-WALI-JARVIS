...
class TaskManager extends ComponentContract {
    constructor(config = {}) {
        ...
        this.allowedTransitions = {
            ...
            COMPLETED: new Set(), FAILED: new Set(["READY"]), CANCELLED: new Set()
        };
        this.loadTasks();
    }
    loadTasks() {
        ...
        } catch (error) { throw new Error(`Failed to load task storage: ${error.message}`); }
    }
    saveTasks() {
    ...
    createTask(input = {}) {
        ...
        const task = { task_id: taskId, intent: input.intent || "", action: input.action || "", target: input.target || null, data: input.data || {}, priority: input.priority || "NORMAL", status: "CREATED", dependencies: Array.isArray(input.dependencies) ? [...new Set(input.dependencies)] : [], created_at: now, updated_at: now, audit: [{ from: null, to: "CREATED", timestamp: now, reason: "TASK_CREATED" }] };
        this.tasks.set(taskId, task); this.saveTasks(); return task;
    }
    ...
    updateTaskStatus(taskId, status, metadata = {}) {
        ...
        if (previousStatus !== status && !this.allowedTransitions[previousStatus]?.has(status)) throw new Error(`Invalid task transition: ${previousStatus} -> ${status}`);
        const now = new Date().toISOString(); task.status = status; Object.assign(task, metadata); task.updated_at = now;
        if (!Array.isArray(task.audit)) task.audit = [];
        if (previousStatus !== status) task.audit.push({ from: previousStatus, to: status, timestamp: now, reason: metadata.reason || metadata.request_id || null, ...(metadata.request_id ? { request_id: metadata.request_id } : {}) });
        this.saveTasks(); return task;
    }
    addDependency(taskId, dependencyId) {
        const task = this.getTask(taskId); if (!task) throw new Error(`Unknown task: ${taskId}`);
        ...
        task.updated_at = new Date().toISOString(); this.saveTasks(); return task;
    }
    ...
    retryTask(taskId, metadata = {}) {
        const task = this.getTask(taskId); if (!task) throw new Error(`Unknown task: ${taskId}`);
        if (task.status !== "FAILED") throw new Error(`Task is not retryable: ${taskId}`);
        const retryCount = Number(task.retry_count || 0) + 1;
        return this.updateTaskStatus(taskId, "READY", { retry_count: retryCount, last_retry_at: new Date().toISOString(), ...metadata });
    }
    ...
    getSummary() { const byStatus = {}; for (const task of this.listTasks()) byStatus[task.status] = (byStatus[task.status] || 0) + 1; return { total: this.tasks.size, by_status: byStatus }; }
    execute(input = {}) {
        switch (input.action) {
            ...
            case "RETRY_TASK": return { success: true, task: this.retryTask(input.task_id, input.metadata) };
